...
const BUTTON_GRAY = "#9a9a9a";
const BUTTON_GRAY2 = "#bbbbbb";
const LIGHT_BLUE = "#33ccff";
const LIGHTEST_BLUE = "#b3edff";
const MODDABLE_BLUE = "#1932ab";
const GREEN = "#0e8700";
const RED = "#990000";
...
const LightestBlueSkin = Skin.template({ fill: LIGHTEST_BLUE });

const OpenSans24 = Style.template({ color: BLACK, font: "24px Open Sans" });
const OpenSans24White = Style.template({ color: WHITE, font: "24px Open Sans", horizontal: "left" });
const OpenSans20 = Style.template({ color: BLACK, font: "20px Open Sans" });
const OpenSans20Left = Style.template({ color: BLACK, font: "20px Open Sans", horizontal: "left" });
const OpenSans20White = Style.template({ color: WHITE, font: "20px Open Sans", horizontal: "left" });
...
const DeviceTexture = Texture.template({ path:"device.png" });
const DeviceSkin = Skin.template({ 
  Texture: DeviceTexture, color: WHITE,
  x: 0, y: 0, width: 36, height: 36,
});

const ResetTexture = Texture.template({ path:"reset.png" });
const ResetSkin = Skin.template({ 
  Texture: ResetTexture, color: WHITE,
  x: 0, y: 0, width: 48, height: 48,
});

const RunTexture = Texture.template({ path:"run.png" });
const RunSkin = Skin.template({ 
  Texture: RunTexture, color: WHITE,
  x: 0, y: 0, width: 38, height: 42,
});

const WiFiTexture = Texture.template({ path:"wifi.png" });
const WiFiSkin = Skin.template({ 
  Texture: WiFiTexture, color: WHITE,
  x: 0, y: 0, width: 42, height: 30,
});

const ButtonFillSkin = Skin.template({ 
  fill: [MODDABLE_BLUE, BUTTON_GRAY, GREEN, RED]
});
...
export const WiFiStatusSpinner = Container.template($ => ({
  ...
  contents: [
    ...
    Label($, {
      bottom: 30, height: 18, left:0, right: 0, Style: OpenSans20, string: $.status,
    }),
  ],
}));
