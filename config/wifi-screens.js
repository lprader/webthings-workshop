...
class BackToMainBehavior extends Behavior {
...
export const NetworkListScreen = Container.template($ => ({ 
	...
	contents: [
		...
		new ASSETS.Header({ title: "Networks", backArrowBehavior: BackToMainBehavior }),
	],
}));
...
const KeyboardContainer = Column.template($ => ({
	...
	contents:[
		KeyboardField($, {
			...
			Skin: ASSETS.WhiteSkin, Style: ASSETS.OpenSans20Left, visible: false
		}),
		Container($, {
	...
	Behavior: class extends Behavior {
		...
		addKeyboard() {
			this.data["KEYBOARD"].add(HorizontalExpandingKeyboard(this.data, {
				style:new ASSETS.OpenSans20Left(), target:this.data["FIELD"], doTransition:TRANSITION
			}));
		}
	}
}));
...
export const ConnectionErrorScreen = Column.template($ => ({
	left: 0, right: 0, top: 0, bottom: 0, Skin: ASSETS.WhiteSkin, Style: ASSETS.OpenSans20, contents: [
		...
		Text($, {
			top: 20, height: 32, left: 0, right: 0, Style: ASSETS.OpenSans20,
